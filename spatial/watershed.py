...
a.add_argument('--figure_log_root',
               default='/cluster/scratch/jingyli/interpretable/visualization', type=str,
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(22)):
        for j in range(22):
            ...
            filename = f"{DATE}_{TIME}_C{CHANNEL}-W{W}" if args.feat_index == 11 else f"{DATE}_{TIME}_C{CHANNEL}-W{W}-feat{args.feat_index}"

            space_mat = get_space_mat(attr)
            # of shape [19,512,448]
            # 19 = 12 timeepochs + 7 static features

            space_mat_ = normalize_space_mat(space_mat, log_norm=args.attr_to_log)
            ...
            # Parameters
            BINARY_THRED = args.binary_thred
            DIST_PEAK_THRED = args.dist_peak_thred
